"use strict";
const commentBubbles = document.querySelectorAll(".comment-bubble") as NodeListOf<HTMLElement>;
...
function SetActive(index:number, activate:boolean) {
    // console.log(`${index} set to ${activate? "active":"inactive"}`);
    if (activate)
        activeIndex = index;
    ToggleOverlay(commentBubbles[index], !activate);
}
function ToggleOverlay(element:HTMLElement, turnON:boolean) {
